# NO.27 Python SendEmail

这是从Sdp抠出来的模块，参考某大神的说明，将发送邮件功能作为类，调用时实例化即可，用send方法调用发送函数，参数是user、user_email、正文，如果是localhost，请修改析构函数，去掉self.smtp_server, self.password, self.from_addr。
